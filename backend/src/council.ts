 ...
 */
export async function stage1CollectResponses(userQuery: string): Promise<ModelResponse[]> {
...
export async function stage2CollectRankings(
  userQuery: string,
  stage1Results: ModelResponse[]
): Promise<{ rankings: RankingResponse[], labelToModel: Record<string, string> }> {
...
export function calculateAggregateRankings(
  ...
  for (const [model, positions] of Object.entries(modelPositions)) {
    if (positions.length > 0) {
      const avgRank = positions.reduce((sum, pos) => sum + pos, 0) / positions.length;
      aggregate.push({
        model,
...
export async function runFullCouncil(userQuery: string): Promise<{
  ...
  const { rankings: stage2Results, labelToModel } = await stage2CollectRankings(userQuery, stage1Results);
